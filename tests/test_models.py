...
class TestProductModel(unittest.TestCase):
    ...
    def test_deserialize_with_no_name(self):
        """It should not deserialize a Product with no name"""
        data = {
            "description": "testing",
            "price": 123.45,
            "available": True,
    ...
    def test_update_with_no_id(self):
        """It should raise an error when trying to update without id"""
        product = ProductFactory()
        product.id = None
    ...
    def test_find_by_price_string(self):
        ...
        for product in products:
            product.create()
        price = products[0].price
        price_str = f'"{str(price)}"'  # Format as string with quotes
        found = Product.find_by_price(price_str)
        self.assertGreater(found.count(), 0)
        for product in found:
            self.assertEqual(product.price, Decimal(price_str.strip(' "')))
